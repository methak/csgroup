...
local function lock_group_name(msg, data, target)
    ...
    local group_name_lock = data[tostring(target)]['settings']['lock_name']
        if group_name_lock == 'yes' then
            return 'Group name is already lockedopen by @kinghazrat '
        else
            data[tostring(target)]['settings']['lock_name'] = 'yes'
                save_data(_config.moderation.data, data)
                rename_chat('chat#id'..target, group_name_set, ok_cb, false)
        return 'Group name has been locked open by @kinghazrat'
    end
end
...
local function unlock_group_name(msg, data, target)
    ...
    local group_name_lock = data[tostring(target)]['settings']['lock_name']
        if group_name_lock == 'no' then
            return 'Group name is already unlockedopen by @kinghazrat  '
        else
            data[tostring(target)]['settings']['lock_name'] = 'no'
            save_data(_config.moderation.data, data)
        return 'Group name has been unlockedopen by @kinghazrat '
    end
end
...
local function lock_group_member(msg, data, target)
    ...
    local group_member_lock = data[tostring(target)]['settings']['lock_member']
        if group_member_lock == 'yes' then
            return 'Group members are already lockedopen by @kinghazrat '
        else
            data[tostring(target)]['settings']['lock_member'] = 'yes'
            save_data(_config.moderation.data, data)
        end
        return 'Group members has been lockedopen by @kinghazrat '
end
...
local function unlock_group_member(msg, data, target)
    ...
    local group_member_lock = data[tostring(target)]['settings']['lock_member']
        if group_member_lock == 'no' then
            return 'Group members are not lockedopen by @kinghazrat '
        else
            data[tostring(target)]['settings']['lock_member'] = 'no'
            save_data(_config.moderation.data, data)
        return 'Group members has been unlocked open by @kinghazrat'
	end
end
...
local function lock_group_photo(msg, data, target)
    ...
    local group_photo_lock = data[tostring(target)]['settings']['lock_photo']
        if group_photo_lock == 'yes' then
            return 'Group photo is already lockedopen by @kinghazrat '
        else
...
local function unlock_group_photo(msg, data, target)
    ...
    local group_photo_lock = data[tostring(target)]['settings']['lock_photo']
        if group_photo_lock == 'no' then
            return 'Group photo is not lockedopen by @kinghazrat '
        else
            data[tostring(target)]['settings']['lock_photo'] = 'no'
            save_data(_config.moderation.data, data)
        return 'Group photo has been unlocked open by @kinghazrat'
	end
end
...
local function lock_group_flood(msg, data, target)
    ...
    local group_flood_lock = data[tostring(target)]['settings']['flood']
        if group_flood_lock == 'yes' then
            return 'Group flood is locked open by @kinghazrat'
        else
            data[tostring(target)]['settings']['flood'] = 'yes'
            save_data(_config.moderation.data, data)
        return 'Group flood has been locked open by @kinghazrat '
	end
end
...
local function unlock_group_flood(msg, data, target)
    ...
    local group_flood_lock = data[tostring(target)]['settings']['flood']
        if group_flood_lock == 'no' then
            return 'Group flood is not locked open by @kinghazrat'
        else
            data[tostring(target)]['settings']['flood'] = 'no'
            save_data(_config.moderation.data, data)
        return 'Group flood has been unlocked open by @kinghazrat'
	end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'Arabic is already locked open by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Arabic has been locked open by @kinghazrat'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'Arabic/Persian is already unlockedopen by @kinghazrat '
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Arabic/Persian has been unlocked open by @kinghazrat'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return 'RTL char. in names is already locked open by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'RTL char. in names has been locked open by @kinghazrat'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return 'RTL char. in names is already unlocked open by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return 'RTL char. in names has been unlocked open by @kinghazrat'
  end
end
...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return 'Link posting is already locked open by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Link posting has been lockedopen by @kinghazrat '
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'Link posting is not locked open by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Link posting has been unlocked open by @kinghazrat'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return 'SuperGroup spam is already locked open by @kinghazrat '
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'SuperGroup spam has been locked open by @kinghazrat'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return 'SuperGroup spam is not locked open by @kinghazrat '
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return 'SuperGroup spam has been unlocked open by @kinghazrat'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'Sticker posting is already locked open by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Sticker posting has been locked open by @kinghazrat'
  end
end
...
local function unlock_group_sticker(msg, data, target)
	...
	if group_sticker_lock == 'no' then
		return 'Sticker posting is already unlocked open by @kinghazrat'
	else
		data[tostring(target)]['settings']['lock_sticker'] = 'no'
		save_data(_config.moderation.data, data)
		return 'Sticker posting has been unlocked open by @kinghazrat'
	end
end
