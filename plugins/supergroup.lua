...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return 'Link posting is already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Link posting has been locked Moded by @kinghazrat'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'Link posting is not locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Link posting has been unlocked Moded by @kinghazrat'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return 'SuperGroup spam is already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'SuperGroup spam has been locked Moded by @kinghazrat'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return 'SuperGroup spam is not locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return 'SuperGroup spam has been unlocked Moded by @kinghazrat'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'Flood is already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Flood has been locked Moded by @kinghazrat'
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'Flood is not locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Flood has been unlocked Moded by @kinghazrat'
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'Arabic is already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Arabic has been locked Moded by @kinghazrat'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'Arabic/Persian is already unlocked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Arabic/Persian has been unlocked Moded by @kinghazrat'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'SuperGroup members are already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'SuperGroup members has been locked Moded by @kinghazrat'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'SuperGroup members are not locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'SuperGroup members has been unlocked Moded by @kinghazrat'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return 'RTL is already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'RTL has been locked Moded by @kinghazrat'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return 'RTL is already unlocked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return 'RTL has been unlocked Moded by @kinghazrat'
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return 'Tgservice is already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Tgservice has been locked Moded by @kinghazrat'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return 'TgService Is Not Locked! Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Tgservice has been unlocked Moded by @kinghazrat'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'Sticker posting is already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Sticker posting has been locked Moded by @kinghazrat'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return 'Sticker posting is already unlocked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Sticker posting has been unlocked Moded by @kinghazrat'
  end
end
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return 'Contact posting is already locked Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Contact posting has been locked Moded by @kinghazrat'
  end
end
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return 'Contact posting is already unlocked Moded by @kinghazrat'
  else
   d data[tostring(target)]['settings']['lock_contacts'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Contact posting has been unlocked Moded by @kinghazrat'
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    return 'Settings are already strictly enforced Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['strict'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'Settings will be strictly enforced Moded by @kinghazrat'
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return 'Settings are not strictly enforced Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return 'Settings will not be strictly enforced Moded by @kinghazrat'
  end
end
...
local function set_public_membermod(msg, data, target)
  if not is_momod(msg) then
    return "For moderators only! Moded by @kinghazrat"
  end
  ...
  if group_public_lock == 'yes' then
    return 'Group is already public Moded by @kinghazrat'
  else
...
local function unset_public_membermod(msg, data, target)
  ...
  if group_public_lock == 'no' then
    return 'Group is not public Moded by @kinghazrat'
  else
    data[tostring(target)]['settings']['public'] = 'no'
	data[tostring(target)]['long_id'] = msg.to.long_id
    save_data(_config.moderation.data, data)
    return 'SuperGroup is now: not public Moded by @kinghazrat'
  end
end
...
local function promote2(receiver, member_username, user_id)
  ...
  local member_tag_username = string.gsub(member_username, '@', '(at)')
  if not data[group] then
    return send_large_msg(receiver, 'SuperGroup is not added.Moded by @kinghazrat')
  end
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..' is already a moderator.')
  end
...
local function demote2(receiver, member_username, user_id)
  ...
  local group = string.gsub(receiver, 'channel#id', '')
  if not data[group] then
    return send_large_msg(receiver, 'Group is not added.Moded by @kinghazrat')
  end
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_tag_username..' is not a moderator.')
  end
...
local function modlist(msg)
  ...
  if not data[tostring(groups)][tostring(msg.to.id)] then
    return 'SuperGroup is not added.Moded by @kinghazrat'
  end
...
local function callbackres(extra, success, result)
  ...
  elseif get_cmd == "invite" then
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			unmute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] removed from muted user list Moded by @kinghazrat")
		elseif is_owner(extra.msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] added to muted user list Moded by @kinghazrat")
		end
	end
end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'SuperGroup is already added. Moded by @kinghazrat', ok_cb, false)
			end
			print("SuperGroup "..msg.to.print_name.."("..msg.to.id..") added Moded by @kinghazrat")
			superadd(msg)
			set_mutes(msg.to.id)
		...
		if matches[1] == 'rem' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'SuperGroup is not added.Moded by @kinghazrat', ok_cb, false)
			end
			print("SuperGroup "..msg.to.print_name.."("..msg.to.id..") removed Moded by @kinghazrat")
			superrem(msg)
			rem_mutes(msg.to.id)
		end
		...
		if msg.text then
			if msg.text:match("^(https://telegram.me/joinchat/%S+)$") msg.text:match("^(https://t.me/joinchat/%S+)$") and data[tostring(msg.to.id)]['settings']['set_link'] == 'waiting' and is_owner(msg) then
